...
contract('ZENXToken', function (accounts) {
    ...
    let totalsupply = 2300000000;

    beforeEach(async function () {
        const { instances } = await setUpUnitTest(accounts);
        token = instances.ZENXToken;
    });
    ...
    describe('recover', function () {
        it('recovers the tokens sent to the ZENXToken address', async function () {
            const amount = web3.utils.toWei(new BN(totalsupply));
